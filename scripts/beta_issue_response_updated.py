...
"""
            ...
            # 전화 알림 (새로 추가됨)
            if "phone" in channels and self.config["notifications"]["phone"]["enabled"]:
                try:
                    self.send_phone_alert(alert)
                except Exception as e:
                    log.error(f"전화 알림 전송 실패: {e}")
        except Exception as e:
    ...
    def create_jira_issue(self, alert):
        ...
        # 프로젝트 키 가져오기
        project_key = self.config["response_actions"][alert["severity"]]["jira"]["project_key"]
        ...
        # JIRA API 호출
        api_version = jira_config.get("api_version", "2")
        api_endpoint = jira_config.get("api_endpoint", "/rest/api/2/issue")
        url = f"{jira_config['url']}{api_endpoint}"
