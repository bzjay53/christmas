-- 🎄 Christmas Trading: AI 테이블 스키마 최종 수정
-- 실행일: 2025-05-30
...
-- strategy_type 값 제한
ALTER TABLE ai_learning_data 
ADD CONSTRAINT IF NOT EXISTS chk_ai_learning_strategy_type 
CHECK (strategy_type IN ('traditional', 'ai_learning', 'hybrid', 'custom'));

ALTER TABLE ai_strategy_performance 
ADD CONSTRAINT IF NOT EXISTS chk_ai_performance_strategy_type 
CHECK (strategy_type IN ('traditional', 'ai_learning', 'hybrid', 'custom'));

-- learning_phase 값 제한
ALTER TABLE ai_learning_data 
ADD CONSTRAINT IF NOT EXISTS chk_learning_phase 
CHECK (learning_phase IN ('training', 'validation', 'production'));

-- confidence_score 범위 제한
ALTER TABLE ai_learning_data 
ADD CONSTRAINT IF NOT EXISTS chk_confidence_score 
CHECK (confidence_score BETWEEN 0.0 AND 1.0);

-- feedback_score 범위 제한
ALTER TABLE ai_learning_data 
ADD CONSTRAINT IF NOT EXISTS chk_feedback_score 
