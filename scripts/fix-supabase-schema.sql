...
-- 2. ai_strategy_performance 테이블에 strategy_type 컬럼 추가
ALTER TABLE ai_strategy_performance 
ADD COLUMN IF NOT EXISTS strategy_type VARCHAR(50) DEFAULT 'traditional';

-- 3. 기존 데이터 업데이트 (NULL 값을 'traditional'로 설정)
...
-- 7. 컬럼 설명 추가 (문서화)
COMMENT ON COLUMN ai_learning_data.strategy_type IS 'AI 전략 타입: traditional(전통적), ai_learning(AI학습), hybrid(하이브리드), custom(사용자정의)';
COMMENT ON COLUMN ai_strategy_performance.strategy_type IS 'AI 전략 타입: traditional(전통적), ai_learning(AI학습), hybrid(하이브리드), custom(사용자정의)';
