...
export const CryptoCard: React.FC<CryptoCardProps> = ({ 
  ...
  return (
    <div 
      className={`
        crypto-card rounded-xl p-6 cursor-pointer bg-gray-900/60 backdrop-blur-sm
        border border-gray-700/50 hover:border-green-500/50
        transform transition-all duration-300 hover:scale-105
        ${isPositive ? 'shadow-green-500/20' : isNeutral ? 'shadow-yellow-500/20' : 'shadow-red-500/20'} shadow-lg
        ${isAnimating ? (isPositive ? 'animate-pulse' : 'animate-pulse') : ''}
        ${className}
      `}
      onClick={onClick}
    >
